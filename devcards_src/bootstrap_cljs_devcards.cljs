...
(defcard ex-6
  (react-card
    ...
    (bs/button-group
      (for [style styles]
        (bs/dropdown-button {:title (str/capitalize style) :bs-style style}
    ...
    (bs/button-toolbar
      (for [style styles]
        (bs/split-button {:title (str/capitalize style) :bs-style style}
          (map #(bs/menu-item {:key (str %)} (str "Link " %)) (range 1 5)))))
    ))
  ...
  (react-card
    ...
    (bs/button-toolbar
      (map
        #(bs/overlay-trigger
           {:placement %
            :overlay (bs/tooltip "Check this info!")}
    ...
    (bs/button-toolbar
      (map
        #(bs/overlay-trigger
           {:trigger "click"
            :placement %
            :overlay (bs/popover {:title (str "Popover " %)}
  ...
  (react-card
    ...
    (bs/navbar
      (bs/nav
        ...
        (bs/dropdown-button {:key 3 :title "Dropdown"}
    ...
    (dom/hr "")
    (mkdn "### Tabs")
    (bs/tabbed-area {:default-active-key 2}
      (map #(bs/tab-pane
              {:key %
               :tab (str "Tab " %)}
    ...
    (mkdn "### Pagers")
    (bs/pager
      (bs/page-item {:href "#!/bootstrap_cljs_devcards/ex-12"} "Previous")
      (bs/page-item {:href "#!/bootstrap_cljs_devcards/ex-14"} "Next"))))
  ...
  (react-card
    ...
    (dom/div
      (dom/form
        ...
        (bs/input {:type "static" :value "static"})))
    (dom/hr "")
    (mkdn "## Addons")
